...
if (isset($_GET['delete_direccion'])) {
    ...
    // Verificar si la dirección está siendo usada por alguna relación en clientes_direcciones
    $check_clientes_direcciones_stmt = $conn->prepare("SELECT COUNT(*) AS count FROM clientes_direcciones WHERE direccion_id = ?");
    $check_clientes_direcciones_stmt->bind_param("i", $direccion_id);
    $check_clientes_direcciones_stmt->execute();
    $check_clientes_direcciones_result = $check_clientes_direcciones_stmt->get_result();
    $row_cd = $check_clientes_direcciones_result->fetch_assoc();
    $check_clientes_direcciones_stmt->close();

    if ($row_cd['count'] > 0) {
        $message = "<p class='error'>No se puede eliminar la dirección porque está asociada a uno o más clientes.</p>";
    } else {
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="direcciones.php?delete_direccion=<?php echo $row['direccion_id']; ?>" onclick="return confirm('¿Estás seguro de que quieres eliminar esta dirección? ¡Solo se podrá eliminar si no está asociada a ningún cliente!');">Eliminar</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
